...
核心实现是构建检索链，你可以使用RetrievalQA或ConversationalRetrievalChain。检索过程中，用户查询首先被转换为向量，然后在向量数据库中找到最相似的文档片段，这些片段作为上下文传递给LLM。LangChain提供了VectorStoreRetriever来处理这个检索过程，你可以配置返回的文档数量和相似度阈值。
...
// 第一步：文档加载和预处理
DocumentLoader loader = new TextLoader("product_manuals.txt");
...
LLMChain llmChain = new LLMChain(
    new ChatOpenAI().setTemperature(0.1),
    promptTemplate
);

RetrievalQA ragChain = new RetrievalQA(llmChain, retriever);
