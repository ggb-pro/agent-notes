...
public class SelfRefinementAgent {
    ...
    public SelfRefinementAgent(LLMClient llmClient) {
    ...
    private CriticResult parseEvaluation(String jsonResponse) {
        ...
        try {
            ...
            return new CriticResult(score, issues);
        } catch (Exception e) {
            // 解析失败时返回保守评估
            return new CriticResult(0.5, List.of("无法解析评估结果"));
        }
    }

    // 评估结果封装类
    static class CriticResult {
        private final double score;
        private final List<String> issues;

        public CriticResult(double score, List<String> issues) {
            this.score = score;
            this.issues = issues;
        }
