...
public class LoopState {
    ...
    public LoopState updateIteration() {
...
public void handleLoopError(Exception error, GraphState state) {
    if (error instanceof RetryableException) {
        if (state.getRetryCount() < MAX_RETRIES) {
            return scheduleRetry(state.incrementRetry());
        }
    }

    if (error instanceof CriticalException) {
        return terminateWithError(error, state);
    }

    // 对于未知异常，采用保守策略
    return gracefulDegradation(state);
}
实践应用场景
...
public GraphState optimizeStateSize(GraphState state) {
